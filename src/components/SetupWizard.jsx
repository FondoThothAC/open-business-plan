    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          ...
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '1.75rem', fontWeight: 700, marginBottom: '0.5rem', color: 'var(--text-primary)' }}>
        ...
        {step === 0 && (
          <div style={{ animation: 'fadeIn 0.3s ease' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '2rem' }}>
              {[
                { icon: Brain, title: 'Mesa de Expertos', desc: '3 agentes IA: Analista → Crítico → Redactor', color: 'var(--accent-color)' },
                { icon: Monitor, title: 'IA Local (Ollama)', desc: 'Privado, ilimitado, sin costo de API', color: 'var(--success-color)' },
                { icon: Cloud, title: 'Fallback en Nube', desc: 'Groq, Gemini, Mistral como respaldo', color: 'var(--accent-color)' },
                { icon: Zap, title: '256k de Contexto', desc: 'El plan completo cabe en una sola sesión', color: 'var(--accent-color)' },
                ...
                <div key={title} style={{
                  ...
                  <div style={{ fontWeight: 700, fontSize: '0.85rem', marginBottom: '0.25rem', color: 'var(--text-primary)' }}>{title}</div>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>{desc}</div>
                </div>
              ))}
            </div>
            <button className="btn btn-ia" style={{ width: '100%', padding: '0.9rem', fontSize: '1rem', height: '3rem' }}
              onClick={() => setStep(1)}>
        ...
        {step === 1 && (
          <div style={{ animation: 'fadeIn 0.3s ease' }}>
            <h2 style={{ fontFamily: 'var(--font-display)', marginBottom: '0.5rem', fontSize: '1.2rem', color: 'var(--text-primary)' }}>
              ¿Dónde corre la IA?
            </h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '2rem' }}>
              {[
                { val: 'local', icon: Server, label: 'Local (Ollama)', desc: 'RTX, GPU, sin costo de API', color: 'var(--success-color)', badge: 'Recomendado' },
                { val: 'cloud', icon: Cloud, label: 'Solo Nube', desc: 'Groq, Gemini, OpenAI', color: 'var(--accent-color)', badge: null },
              ].map(({ val, icon: Icon, label, desc, color, badge }) => (
                <button key={val}
                  onClick={() => { setMode(val); setStep(val === 'local' ? 2 : 3); }}
                  style={{
                    padding: '1.25rem', borderRadius: '8px', border: '2px solid',
                    borderColor: mode === val ? 'var(--accent-color)' : 'var(--border-color)',
                    background: 'var(--bg-panel-hover)',
                  ...
                  {badge && (
                    <span style={{
                      ...
                      fontSize: '0.6rem', fontWeight: 800, background: 'var(--success-color)',
        ...
        {step === 2 && (
          <div style={{ animation: 'fadeIn 0.3s ease' }}>
            ...
            {error && (
              <div style={{ padding: '1rem', background: 'rgba(239,68,68,0.1)', borderRadius: '8px', border: '1px solid rgba(239,68,68,0.2)', marginBottom: '1rem', fontSize: '0.85rem', color: '#ef4444', display: 'flex', gap: '0.5rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '1.5rem' }}>
                {[
                  ...
                  <div key={label} style={{
                    ...
                    <span style={{ fontSize: '0.8rem', fontWeight: 700, color: ok ? 'var(--success-color)' : 'var(--danger-color)' }}>{value}</span>
                  </div>
                ))}
                <div style={{ marginTop: '0.5rem', padding: '0.75rem 1rem', borderRadius: '8px', background: 'var(--bg-panel-hover)', border: '1px solid var(--border-color)', fontSize: '0.8rem', color: 'var(--accent-color)', fontWeight: 700 }}>
                  💡 Recomendación: <span style={{ color: 'var(--text-primary)' }}>
            ...
            {!detecting && (
              <div style={{ display: 'flex', gap: '0.75rem' }}>
                ...
                <button className="btn btn-ia" style={{ flex: 2, height: '2.5rem' }} onClick={() => setStep(3)}>
        ...
        {step === 3 && (
          <div style={{ animation: 'fadeIn 0.3s ease' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.6rem', marginBottom: '1.5rem' }}>
              {CTX_OPTIONS.map(opt => (
                <button key={opt.value}
                  ...
                  style={{
                    padding: '0.75rem 0.5rem', borderRadius: '8px', border: '2px solid',
                    borderColor: selectedCtx === opt.value ? opt.color : 'var(--border-color)',
                    background: selectedCtx === opt.value ? `${opt.color}15` : 'var(--bg-panel-hover)',
            ...
            <button className="btn btn-ia" style={{ width: '100%', padding: '0.9rem', height: '3rem' }} onClick={() => setStep(4)}>
          ...
          <div style={{ animation: 'fadeIn 0.3s ease', textAlign: 'center' }}>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', marginBottom: '1.5rem' }}>
              Open Business Plan está configurado con <strong style={{ color: 'var(--accent-color)' }}>
                {CTX_OPTIONS.find(o => o.value === selectedCtx)?.label} de contexto
              </strong> corriendo en modo <strong style={{ color: 'var(--success-color)' }}>{mode === 'local' ? 'Local (Ollama)' : 'Nube'}</strong>.
            </p>
            <div style={{ background: 'var(--bg-panel-hover)', border: '1px solid var(--border-color)', borderRadius: '8px', padding: '1rem', marginBottom: '1.5rem', fontSize: '0.8rem', textAlign: 'left', lineHeight: 1.7 }}>
              <strong>Resumen de configuración:</strong><br/>
              🧠 Modelo: <code style={{ color: 'var(--accent-color)' }}>{selectedModel}</code><br/>
              📏 Contexto: <code style={{ color: 'var(--accent-color)' }}>{selectedCtx.toLocaleString()} tokens</code><br/>
              ⚡ Mesa de Expertos: <code style={{ color: 'var(--success-color)' }}>3 fases activas</code><br/>
              🔄 Fallback: <code style={{ color: 'var(--accent-color)' }}>Groq → Gemini → Mistral</code>
            </div>
            <button className="btn btn-ia" style={{ width: '100%', padding: '0.9rem', fontSize: '1rem', height: '3rem' }}
